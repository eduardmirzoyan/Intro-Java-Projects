...
//	public static String test(char letter) {
//		s = letter.replaceFirst("0", "a");
//		return s;
//	}
//	
	public static String allWordsWith(String s) {
		s = s.trim();
		s = s + " ";
		String ptext = "";
		String test = "";
		...
		for(int count = 0; count < s.length(); count++) {
			if(start != -1) {
				test = s.substring(start, end);
				if(test.contains("e") == true) {
					ptext += s.substring(start, end);
				}
				start = end + 1;
